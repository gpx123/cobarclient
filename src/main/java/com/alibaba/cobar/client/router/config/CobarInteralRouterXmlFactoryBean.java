...
public class CobarInteralRouterXmlFactoryBean extends
    ...
    protected void assembleRulesForRouter(
        ...
        for (InternalRule rule : rules) {
            ...
            String fragmentExpression = StringUtils.trimToEmpty(rule.getFragmentExpression());
            ...
            if (StringUtils.isNotEmpty(namespace)) {
                if (StringUtils.isEmpty(shardingExpression)) {
                    ...
                    IBatisNamespaceShardingRule insr;
                    if (StringUtils.isEmpty(fragmentExpression)) {
                        insr = new IBatisNamespaceShardingRule(namespace,
                                destinations, shardingExpression);
                    } else {
                        insr = new IBatisNamespaceShardingRule(namespace,
                                destinations, shardingExpression, fragmentExpression);
                    }
            ...
            if (StringUtils.isNotEmpty(sqlAction)) {
                if (StringUtils.isEmpty(shardingExpression)) {
                    ...
                    IBatisSqlActionShardingRule issr ;
                    if (StringUtils.isEmpty(fragmentExpression)) {
                        issr = new IBatisSqlActionShardingRule(sqlAction,
                                destinations, shardingExpression);
                    } else {
                        issr = new IBatisSqlActionShardingRule(sqlAction,
                                destinations, shardingExpression,fragmentExpression);
